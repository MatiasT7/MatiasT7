    ...
    for (const [ingrediente, info] of Object.entries(ingredientes)) {
        if (ingrediente === 'Variantes') {
            ...
            for (const [variante, varianteInfo] of Object.entries(info)) {
                const cantidad = varianteInfo.cantidad === 'c/n' ? 'c/n' : Math.round(parseFloat(varianteInfo.cantidad) * cantidadComensales);
                html += `<tr><td>${variante}</td><td>${cantidad}</td><td>${varianteInfo.unidad}</td></tr>`;
            }
        } else {
            const cantidad = info.cantidad === 'c/n' ? 'c/n' : Math.round(parseFloat(info.cantidad) * cantidadComensales);
